        ...
        stage('Preparation') {
            steps {
                script {
                    ...
                    sh "echo holaaa"
                    def props = readJSON(text:sh(script:'curl -s "https://cobre-utils.s3.us-east-2.amazonaws.com/pipeline/emisores.json"', returnStdout: true))
                    sh "echo $props"
          ...
          steps {
            script {
              ...
              for (codeInfraEmisor in listEmisores) {
                ...
                sh "docker run --platform linux/amd64 --rm -v $WORKSPACE:/app -w /app node:14-alpine node_modules/.bin/ng build --output-path=${nameOrigin} --base-href=/${nameOrigin}/ --deploy-url /${nameOrigin}/"
              }
            }
          }
        }
